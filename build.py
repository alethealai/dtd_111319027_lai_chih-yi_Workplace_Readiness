...
def chart(series, minimum, maximum, title):
    ...
    return f'<svg class="chart" viewBox="0 0 {w} {h}" role="img" aria-label="{title}，橫軸為遊戲第1至13週，縱軸單位為箱；完整數據見下方表格"><title>{title}</title>'+''.join(lines)+'</svg>'

orders=chart([(3,'#172d46',False),(4,'#a95312',True)],0,100,'工廠收到的需求量與訂購量')
inventory=chart([(2,'#172d46',False)],-100,20,'工廠庫存變化，第8週最低為負95箱')
table_rows=''.join('<tr>'+''.join(f'<td'+(' class="negative"' if isinstance(v,int) and v<0 else '')+f'>{v if v is not None else "未記錄"}</td>' for v in row)+'</tr>' for row in data)
article=f'''
...
<section id="results" class="article-section"><h2>操作與成果</h2><p>照片記錄了第 1 至 13 週的工廠到貨量、庫存量與需求量，以及第 1 至 12 週的訂購量。以下圖表依表格轉錄，沒有使用設計稿的示意數據。</p>
...
<noscript><p class="no-script">完整數據可直接展開下方表格閱讀；啟用 JavaScript 後可切換庫存圖表。</p></noscript>
...
<details class="data-details"><summary>查看完整 13 週數據</summary><div class="table-scroll" role="region" aria-label="工廠週報告數據表，可左右捲動" tabindex="0"><table><caption>工廠週報告 · 單位：箱</caption><thead><tr><th scope="col">遊戲週次</th><th scope="col">到貨量</th><th scope="col">庫存量</th><th scope="col">需求量</th><th scope="col">訂購量</th></tr></thead><tbody>{table_rows}</tbody></table></div><p class="source-note">來源：課堂照片 IMG_8218、IMG_8217。原表以紅色括號呈現負庫存，這裡轉為負號；第 13 週訂購欄為空白，不以 0 代替。</p></details>
<p>從這份紀錄可以觀察到：需求上升、追加訂貨與實際到貨並不同步。這提醒我們，訂貨時需要把時間延遲納入考量，而不是只回應眼前的缺口。</p></section>
<section id="reflection" class="article-section"><h2>我的反思</h2><div class="reflection"><p>這是我第一次接觸這類供應鏈模擬遊戲。一開始，我以為只要顧好自己這一端收到的需求，再決定相對應的訂貨量就可以。實際進行幾週後才發現，下游零售商的需求不斷變動，加上資訊傳遞與到貨都有延遲，我的庫存常在短缺與過剩之間擺盪：需要貨時沒有庫存，不需要時卻累積了很多貨。</p><p>老師提醒我們，訂貨後要兩週才會到貨，而下游的需求傳到上游也需要時間。這讓我明白，做決策不能只看自己眼前收到的需求，還要持續觀察下游的需求趨勢，並把已經下單、尚未到貨的數量一起納入判斷。這次遊戲讓我體會到，供應鏈中每個角色看似各自做決定，其實彼此的決策會互相影響；如果只處理眼前的問題，很容易讓缺貨與庫存過剩反覆出現。</p></div></section>
